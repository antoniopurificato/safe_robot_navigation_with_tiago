...
class CrowdPredictionManager:
    ...
    def __init__(self):
        ...
        self.robot_configuration= crowd_navigation_core.utils.Configuration(
            0.0, 0.0, 0.0
        )
    ...
    def log_values(self):
        ...
        savemat('/home/antonio/Documents/catkin_ws/log_pos_cp.mat', output_dict)

    def unycicle_input_callback(self, geometry_msg):
        linear_velocity = geometry_msg.linear
        angular_velocity = geometry_msg.angular
        tiago_velocity = crowd_navigation_core.utils.TiagoVelocity(
            linear_velocity.x,
            linear_velocity.y,
            angular_velocity.z
        )
    ...
    def laser_scan_callback(self, laser_scan_msg):
        self.data_lock.acquire()
        self.laser_scan = \
            crowd_navigation_core.utils.LaserScan.from_message(laser_scan_msg)
        self.data_lock.release()
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            for i,fsm in enumerate(fsms):
                ...
                elif  fsm.next_time_pass_to is 'start':
            ...
            for i,fsm in enumerate(fsms):
                ...
                pos_i = crowd_navigation_core.utils.Position(
                    current_state[0],
                    current_state[1],
                    0
                )
                vel_i = crowd_navigation_core.utils.Velocity(current_state[2], current_state[3], i)

                crowd_motion_prediction.append(
                    crowd_navigation_core.utils.MotionPrediction(
                        pos_i,
                        vel_i
                    )
                )
                ...
                for  j in range(0,CommonVars.N_PREDICTIONS):
                    ...
                    vel_i = crowd_navigation_core.utils.Velocity(current_state[2], current_state[3], i*10 + j)
            ...
            if CommonVars.FAKE_FSM:
                ...
                crowd_motion_prediction = \
                    crowd_navigation_core.utils.CrowdMotionPrediction()
                for actor_name, actor_configuration in self.actor_configurations.iteritems():
                    
                    if CommonVars.CROWD_PRINT:   rospy.loginfo('{} at configuration {}'.format(actor_name, actor_configuration))
                    crowd_motion_prediction.append(
                    crowd_navigation_core.utils.MotionPrediction(
                        crowd_navigation_core.utils.Position(actor_configuration.x,actor_configuration.y,0),
                        crowd_navigation_core.utils.Velocity(0,0,0)
                    )
                )
            ...
            crowd_motion_prediction_stamped_msg = \
                crowd_navigation_core.utils.CrowdMotionPredictionStamped.to_message(
                    crowd_motion_prediction_stamped
                )
...
def main():
    rospy.init_node('crowd_navigation_crowd_prediction', log_level=rospy.INFO)
